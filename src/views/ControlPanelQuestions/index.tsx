...
const ControlPanelQuestions: React.FC = () => {
    ...
    const sendQuestionsToTheServer = () => {
        ...
        data.questions?.map((q, i) => {
          formData.append(`questions[${i}][text]`, q.text as string);
          (q.picture as File[])?.map((p, j) => {
            formData.append(`questions[${i}][picture][${j}]`, p);
          });

          (q.question_variants as QuestionTaskVariantProps[])?.map((qv, p) => {
            formData.append(`questions[${i}][question_variants][${p}][text]`, qv.text as string);
            (qv.picture as File[])?.map((pv, jv) => {
              formData.append(`questions[${i}][question_variants][${p}][picture][${jv}]`, pv);
            });
          });
        });

        axios.post('/question_blocks', formData).then(r => {
          setIsLoading(false);
